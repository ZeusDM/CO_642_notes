...
The \( k \)-colour Ramsey number \( R_k\left(s_1, \ldots, s_k\right) \) is the smallest \( n \) such that every \( k \)-colouring of the edges of \( K_n \) results in a monochromatic \( K_{s_i} \) in colour \( i \) for some \( i \).

\begin{proposition}[Homework 3, Exercise 4] \label{prop:hw3-ex4}
    Let \( k \) be an positive integer.
    Then,
    \begin{equation}
        2^k \leq R_k(3, \ldots, 3)-1 \leq 1+k\left(R_{k-1}(3, \ldots, 3)-1\right).
    \end{equation}
\end{proposition}

\begin{proof}[Proof of the first inequality]
We display a \(k\)-coloring of a complete graph with \(2^k\) vertices
that contains no monochromatic triangles.

Let \(V = \{0,1\}^k\) be the set of \(k\)-binary strings \(a = a_1a_2\ldots a_k\),
where \(a_i \in \{0, 1\}\).
Note that \(|V| = 2^k\).
Let \(K_V\) be the complete graph with vertex set \(V\).
Let \(f \colon E(K_V) \to [k]\) 
assign to each edge \(ab\) the smallest index \(i\) such that \(a_i \neq b_i\).

Let \(a, b, c \in V\) be distinct vertices.
If \(f(ab) = f(bc) = i\),
then \(a_i \neq b_i\) and \(b_i \neq c_i\).
Since \(a_i, b_i, c_i \in \{0, 1\}\), it follows that \(a_i = c_i\),
and consequently \(f(ac) \neq i\).
Therefore, \(a, b, c\) is not a monochromatic triangle,
and since \(a, b, c\) were arbitrary, the coloring contains no monochromatic triangles.
...
We show that any \( k \)-coloring of the edges of a complete graph with 
\(
n = k\left(R_{k-1}(3, \ldots, 3) - 1\right) + 2
\)
vertices contains a monochromatic triangle.

Let \(f \colon E(K_n) \to [k]\).
Let \( v \in V(K_n) \).
Note that \(d(v) = n-1\).
By the pigeonhole principle, there exists a color \(\alpha\) such that at least
\(
    \left\lceil \frac{n - 1}{k} \right\rceil
\)
edges incident to \( v \) are colored \(\alpha\).
Let \( N \) denote the neighborhood of \( v \) via edges of color \( \alpha \).
Thus, 
\[
|N| \geq \left\lceil \frac{n - 1}{k} \right\rceil \geq R_{k-1}(3, \ldots, 3),
\]
where the last inequality follows from the definition of \( n \).
Let \(H = K_n[N]\) be the complete subgraph of \( K_n \) induced by \( N \).

If there exists an edge \( uw \in E(H) \) such that \( f(uw) = \alpha \),
then the triangle \( \{u, v, w\} \) is monochromatic in color \( \alpha \).
Otherwise,
the \(f\) is a \((k-1)\)-coloring of \(H\), since \(\alpha\) is not used.
Since \( |N| \geq R_{k-1}(3, \ldots, 3) \),
there exists a monochromatic triangle in \(H\) by definition of \( R_{k-1}(3, \ldots, 3) \).
In both cases,
a monochromatic triangle exists in \( K_n \).

Finally,
since any \( k \)-coloring of \( K_n \) contains a monochromatic triangle,
we have
\[
R_k(3, \ldots, 3) \leq k\left(R_{k-1}(3, \ldots, 3) - 1\right) + 2,
\]
which proves the second inequality.
\end{proof}

\begin{proposition}[Homework 3, Exercise 5] \label{prop:hw3-ex5}
    Let \( c \colon \lfloor e \cdot k! \rfloor \to [k] \) be an assignment of classes.%
    Then, the equation \( x+y=z \) is solvable in at least one of the classes;
    that is, there exist \( x, y, z \in C \) satisfying \( x+y=z \) and \( c(x) = c(y) = c(z) \).
\end{proposition}

\begin{proof}
First, we prove by induction that for \( k \geq 1 \),
\(\frac{R_k(3, \dots, 3) - 1}{k!} \leq \sum_{i=0}^k \frac{1}{i!}\).
Let \(k \geq 1\).
If \( k = 1 \), we have
\(\frac{R_1(3) - 1}{1!} = 2 = \frac{1}{0!} + \frac{1}{1!}\),
which satisfies the inequality.

If \(k \geq 2\), assume that the statement holds for \( k - 1 \).
Using Proposition~\ref{prop:hw3-ex4} and the induction hypothesis, we have
\begin{align}
\frac{R_k(3, \dots, 3) - 1}{k!}
&\leq \frac{k \left( R_{k-1}(3, \dots, 3) - 1 \right) + 1}{k!} \\
&= \frac{R_{k-1}(3, \dots, 3) - 1}{(k-1)!} + \frac{1}{k!} \\
&\leq \sum_{i=0}^{k-1} \frac{1}{i!} + \frac{1}{k!} = \sum_{i=0}^k \frac{1}{i!}
\end{align}
Thus, by induction, the inequality holds for all \( k \geq 1 \).

Let \( k \geq 1 \). Observe that
\[\frac{R_k(3, \dots, 3) - 1}{k!} \leq \sum_{i=0}^k \frac{1}{i!} < \sum_{i=0}^\infty \frac{1}{i!} = e,\]
which implies \(e \cdot k! + 1 > R_k(3, \dots, 3)\), and consequently
\begin{equation} \label{eq:5:inequality}
    \lfloor e \cdot k! \rfloor + 1 \geq R_k(3, \dots, 3).
\end{equation}

Now, we return to the original problem.
Let \( c \colon \lfloor e \cdot k! \rfloor \to [k] \) be an assignment of classes.
Define a coloring \( f \) on the complete graph with vertex set \([1 + \lfloor e \cdot k! \rfloor]\) by
\[f(ab) = c(|a - b|).\]
(Note that, for all \( a, b \in [1 + \lfloor e \cdot k! \rfloor] \), \( |a - b| \leq \lfloor e \cdot k! \rfloor \).)

By the definition of Ramsey numbers and Inequality \eqref{eq:5:inequality},
there exists a monochromatic triangle in \(f\),
that is,
there exist vertices \( a > b > c \) such that \(f(ab) = f(bc) = f(ac)\).
Let \( x = a - b \), \( y = b - c \), and \( z = a - c \).
Then, \( x + y = z \) and \(c(x) = c(y) = c(z)\), as desired.
\end{proof}
...
\begin{theorem}[Dirac--Pósa Theorem] \label{thm:dirac-posa}
    Let \(G\) be a graph with \(n\) vertices
    in which
